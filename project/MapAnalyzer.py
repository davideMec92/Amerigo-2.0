...
class MapAnalyzer:
    ...
    def initValues(self):
        ...
        #                               W                       N                       S                   E
        self.robot_matrix = array([[{'x': 3, 'y': 15}, {'x': 15, 'y': 13}, {'x': 0, 'y': 3}, {'x': 13, 'y': 0}],
                                   # DX
        ...
        self.robot_movements_x = [self.robot_matrix[0][2].get('x'), self.robot_matrix[0][2].get('x'),
                                  self.robot_matrix[0][1].get('x'), self.robot_matrix[0][1].get('x')]
        self.robot_movements_y = [self.robot_matrix[0][3].get('y'), self.robot_matrix[0][0].get('y'),
                                  self.robot_matrix[0][0].get('y'), self.robot_matrix[0][3].get('y')]
    ...
    def buildMap(self, filename):
        ...
        try:
            ...
            for line in Lines:
                ...
                if temp_bug_mode_last_move is not None:
                    ...
                    if temp_bug_mode_last_move == 'C':
                        self.actual_orientation = self.actual_orientation.__next__
                ...
                if self.actual_orientation.data == 'N':
                    self.generateObstacleSquarePointsB(
                        self.roundToLowerMultiple(self.left_proximity_actual_x) - temp_left_proximity,
                        self.roundToLowerMultiple(self.left_proximity_actual_y))
                    self.generateObstacleSquarePointsA(self.roundToLowerMultiple(self.front_proximity_actual_x),
                                                       temp_front_proximity + self.front_proximity_actual_y)
                    self.generateObstacleSquarePointsA(
                        self.roundToLowerMultiple(self.right_proximity_actual_x) + temp_right_proximity,
                        self.roundToLowerMultiple(self.right_proximity_actual_y))
                elif self.actual_orientation.data == 'S':
                    self.generateObstacleSquarePointsA(
                        self.roundToLowerMultiple(self.left_proximity_actual_x) + temp_left_proximity,
                        self.roundToLowerMultiple(self.left_proximity_actual_y))
                    self.generateObstacleSquarePointsC(self.roundToLowerMultiple(self.front_proximity_actual_x),
                                                       self.front_proximity_actual_y - temp_front_proximity)
                    self.generateObstacleSquarePointsB(
                        self.roundToLowerMultiple(self.right_proximity_actual_x) - temp_right_proximity,
                        self.roundToLowerMultiple(self.right_proximity_actual_y))
                elif self.actual_orientation.data == 'W':
                    self.generateObstacleSquarePointsC(self.roundToLowerMultiple(self.left_proximity_actual_x),
                                                       self.roundToLowerMultiple(
                                                           self.left_proximity_actual_y) - temp_left_proximity)
                    self.generateObstacleSquarePointsB(
                        self.roundToLowerMultiple(self.front_proximity_actual_x) - temp_front_proximity,
                        self.roundToLowerMultiple(self.front_proximity_actual_y))
                    self.generateObstacleSquarePointsA(self.roundToLowerMultiple(self.right_proximity_actual_x),
                                                       temp_right_proximity + self.roundToLowerMultiple(
                                                           self.right_proximity_actual_y))
                elif self.actual_orientation.data == 'E':
                    self.generateObstacleSquarePointsA(self.roundToLowerMultiple(self.left_proximity_actual_x),
                                                       temp_left_proximity + self.roundToLowerMultiple(
                                                           self.left_proximity_actual_y))
                    self.generateObstacleSquarePointsA(
                        self.roundToLowerMultiple(self.front_proximity_actual_x) + temp_front_proximity,
                        self.roundToLowerMultiple(self.front_proximity_actual_y))
                    self.generateObstacleSquarePointsC(self.roundToLowerMultiple(self.right_proximity_actual_x),
                                                       self.roundToLowerMultiple(
                                                           self.right_proximity_actual_y) - temp_right_proximity)
            ...
            plt.savefig(
                self.map_files_dir_out + '/' + os.path.basename(filename).split('.')[0] + self.file_extension_out,
